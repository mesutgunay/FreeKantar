# FreeKantar: Profesionalni višejezični softver za vage 🚛⚖️🌍

[![EN](https://img.shields.io/badge/Language-EN-blue)](README.md) [![TR](https://img.shields.io/badge/Language-TR-red)](README_TR.md) [![RU](https://img.shields.io/badge/Language-RU-blue)](README_RU.md) [![AR](https://img.shields.io/badge/Language-AR-green)](README_AR.md) [![DE](https://img.shields.io/badge/Language-DE-black)](README_DE.md) [![ES](https://img.shields.io/badge/Language-ES-orange)](README_ES.md) [![RO](https://img.shields.io/badge/Language-RO-blue)](README_RO.md) [![BG](https://img.shields.io/badge/Language-BG-red)](README_BG.md) [![SR](https://img.shields.io/badge/Language-SR-blue)](README_SR.md) [![SQ](https://img.shields.io/badge/Language-SQ-green)](README_SQ.md)

**FreeKantar** je softver za kolske vage visokih performansi, lagan i svestran, dizajniran za moderno industrijsko merenje. Uz podršku za **10 svetskih jezika** i **4 različita formata računa**, pruža besprekorno iskustvo operaterima širom sveta.

---

### 📥 [Preuzmite poslednju verziju (Portable EXE)](https://github.com/mesutgunay/FreeKantar/releases/latest/download/FreeKantar.exe)
*Nije potrebna instalacija. Samo preuzmite i pokrenite.*

---

## 📸 Skenovi ekrana (Screenshots)

![Glavna kontrolna tabla](docs/images/01_main_dashboard.png)
*Moderni interfejs za merenje sa visokom vidljivošću.*

---

## 🔐 Admin režim i upravljanje
Za pristup naprednim funkcijama (upravljanje zalihama, brisanje zapisa):
- **Taster prečica:** `Ctrl + Alt + A`
- **Podrazumevana lozinka:** `AdminMod`

![Mesto za admin panel](docs/images/05_admin_panel.png)
*Potpuna kontrola nad vašim podacima merenja.*

---

## 🌟 Glavne karakteristike
- **🌍 Nativna podrška na 10 jezika:** Potpuno lokalizovan na EN, TR, AR, RU, DE, ES, RO, BG, SR, SQ.
- **📜 Dinamički sistem računa:** Izaberite između:
  - Termalni 80mm / 50mm (Optimizovano za uštedu papira)
  - Standardni račun vage (11x24 cm)
  - A5 Profesionalni horizontalni format
- **🦾 Admin upravljanje:** Napredno upravljanje zalihama (Stok), filtriranje istorijskih podataka i sigurni zapisi.
- **📦 Bez instalacije (Portable):** Jedan fajl `FreeKantar.exe` - samo preuzmite i pokrenite.
- **🛡️ Bezbednost podataka:** Trenutna rezervna kopija i oporavak SQLite baze podataka jednim klikom.
- **🏷️ White-Label računi:** Čist dizajn bez tuđih reklama.

---

## 📥 Kako početi
1. Idite na odeljak **[Releases](https://github.com/mesutgunay/FreeKantar/releases)**.
2. Preuzmite poslednju verziju `FreeKantar.exe`.
3. Povezivanje: Povežite svoj serijski indikator (COM), podesite Baud Rate i počnite sa merenjem!

---

## 🤝 Kontakt i podrška
Za prilagođene korporativne integracije ili premium podršku:
- **WhatsApp:** [+90 533 545 16 80](https://wa.me/905335451680)
- **Email:** [info@otonomkantar.com](mailto:info@otonomkantar.com)
- **Admin vodič:** [Pogledajte tajni admin vodič na 10 jezika](ADMIN_GUIDE.md)

---
