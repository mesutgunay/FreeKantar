# FreeKantar: Softuer Profesional për Peshore në Shumë Gjuhë 🚛⚖️🌍

[![EN](https://img.shields.io/badge/Language-EN-blue)](README.md) [![TR](https://img.shields.io/badge/Language-TR-red)](README_TR.md) [![RU](https://img.shields.io/badge/Language-RU-blue)](README_RU.md) [![AR](https://img.shields.io/badge/Language-AR-green)](README_AR.md) [![DE](https://img.shields.io/badge/Language-DE-black)](README_DE.md) [![ES](https://img.shields.io/badge/Language-ES-orange)](README_ES.md) [![RO](https://img.shields.io/badge/Language-RO-blue)](README_RO.md) [![BG](https://img.shields.io/badge/Language-BG-red)](README_BG.md) [![SR](https://img.shields.io/badge/Language-SR-blue)](README_SR.md) [![SQ](https://img.shields.io/badge/Language-SQ-green)](README_SQ.md)

**FreeKantar** është një softuer për peshore kamionësh me performancë të lartë, i lehtë dhe i gjithanshëm, i projektuar për peshimin industrial modern. Me mbështetje për **10 gjuhë globale** dhe **4 formate të ndryshme faturash**, ai ofron një përvojë të qetë për operatorët në mbarë botën.

---

## 📸 Pamjet e Ekranit (Screenshots)
*(Së shpejti - Vendosni imazhet tuaja në docs/images/)*

![Dashboard Kryesor](docs/images/01_main_dashboard.png)
*Ndërfaqe moderne peshimi me shikueshmëri të lartë.*

---

## 🌟 Karakteristikat Kryesore
- **🌍 Mbështetje Native në 10 Gjuhë:** Plotësisht i lokalizuar në EN, TR, AR, RU, DE, ES, RO, BG, SR, SQ.
- **📜 Sistmi Dinamik i Faturave:** Zgjidhni midis:
  - Termal 80mm / 50mm (I optimizuar për kursimin e letrës)
  - Faturë Standarde Peshoreje (11x24 cm)
  - Format Horizontal Profesional A5
- **🦾 Menaxhimi i Adminit:** Menaxhim i avancuar i stokut (Stok), filtrimi i të dhënave historike dhe regjistrime të sigurta.
- **📦 Pa Instalim (Portable):** Një skedar i vetëm `FreeKantar.exe` - thjesht shkarkoni dhe ekzekutoni.
- **🛡️ Siguria e të Dhënave:** Rezervë dhe rikthim i menjëhershëm i bazës së të dhënave SQLite me një klikim.
- **🏷️ Fatura White-Label:** Dizajn i pastër pa reklama të palëve të treta.

---

## 📥 Si të filloni
1. Shkoni te seksioni **[Releases](https://github.com/yourusername/FreeKantar/releases)**.
2. Shkarkoni versionin më të fundit `FreeKantar.exe`.
3. Lidhja: Lidhni treguesin tuaj serial (COM), vendosni shpejtësinë (Baud Rate) dhe filloni peshimin!

---

## 🤝 Kontakti dhe Mbështetja
Për integrime të personalizuara të ndërmarrjeve ose mbështetje premium:
- **WhatsApp:** [+90 533 545 16 80](https://wa.me/905335451680)
- **Email:** [info@otonomkantar.com](mailto:info@otonomkantar.com)
- **Udhëzuesi i Adminit:** [Shihni udhëzuesin sekret të adminit në 10 gjuhë](ADMIN_GUIDE.md)

---
